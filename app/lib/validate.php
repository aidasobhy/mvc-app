                ...
                foreach ($validationRoles as $validationRole) {
                    ...
                    if (preg_match_all('/(min)\((\d+)\)/', $validationRole, $matches)) {
                        if ($this->min($value, $matches[2][0]) === false) {
                            $this->messenger->add(
                                $this->language->feedKey('text_error_' . $matches[1][0],
                                    [$this->language->get('text_label_' . $fieldName), $matches[2][0]]),
                    ...
                    elseif (preg_match_all('/(lt)\((\d+)\)/', $validationRole, $matches)) {
                        if ($this->lt($value, $matches[2][0]) === false) {
                            $this->messenger->add(
                                $this->language->feedKey('text_error_' . $matches[1][0],
                                    [$this->language->get('text_label_' . $fieldName), $matches[2][0]]),
                                Messenger::APP_MESSAGE_ERROR
                    ...
                    elseif (preg_match_all('/(gt)\((\d+)\)/', $validationRole, $matches)) {
                        if ($this->gt($value, $matches[2][0]) === false) {
                            $this->messenger->add(
                                $this->language->feedKey('text_error_' . $matches[1][0],
                                    [$this->language->get('text_label_' . $fieldName), $matches[2][0]]),
                                Messenger::APP_MESSAGE_ERROR
                    ...
                    elseif (preg_match_all('/(between)\((\d+),(\d+)\)/', $validationRole, $matches)) {
                        if ($this->between($value, $matches[2][0], $matches[3][0]) === false) {
                            $this->messenger->add(
                                $this->language->feedKey('text_error_' . $matches[1][0],
                                    [$this->language->get('text_label_' . $fieldName), $matches[2][0], $matches[3][0]]),
                                Messenger::APP_MESSAGE_ERROR
                            );
                            $errors[$fieldName]=true;
                        }
                    ...
                    elseif (preg_match_all('/(eq)\((\w+)\)/', $validationRole, $matches)) {
                        if ($this->eq($value, $matches[2][0]) === false)  {
                            $this->messenger->add(
                                $this->language->feedKey('text_error_' . $matches[1][0],
                                    [$this->language->get('text_label_' . $fieldName),$this->language->get('text_label_'. $matches[2][0])]),
                                Messenger::APP_MESSAGE_ERROR
                            );
                            $errors[$fieldName]=true;
                        }
                    ...
                    elseif (preg_match_all('/(eq_field)\((\w+)\)/', $validationRole, $matches)) {
                            $otherFieldValue=$inputType[$matches[2][0]];
                        if ($this->eq_field($value,$otherFieldValue) === false)  {
                            $this->messenger->add(
                                $this->language->feedKey('text_error_'.$matches[1][0],
                                    [$this->language->get('text_label_'.$fieldName),
                                        $this->language->get('text_label_'.$matches[2][0])]),
                                Messenger::APP_MESSAGE_ERROR
                            );
                            $errors[$fieldName]=true;
                        }
