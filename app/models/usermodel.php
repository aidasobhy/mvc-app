...
class UserModel extends AbstractModel
{
    ...
    {
        return self::get(
            ' SELECT au.*,aug.group_name FROM ' . self::$tableName . '  au 
            INNER JOIN app_users_groups aug 
            ON aug.group_id=au.group_id
    ...
    public static function authenticate($username, $password,$session)
    {
        ...
        $sql      = ' SELECT *,(SELECT group_name FROM app_users_groups 
                      WHERE app_users_groups.group_id='.self::$tableName.'.group_id) as group_name
                      FROM ' . self::$tableName .
      ...
      if($foundUser !== false){
          ...
          $privileges=UserGroupPrivilegeModel::getBy('group_id');
